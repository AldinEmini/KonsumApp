...
import { MapPin, Phone, Menu, X, ShoppingBag } from 'lucide-react'
...
export default function Header() {
  ...
  const { t } = useI18n()
  useEffect(() => { fetch('/api/content').then(r=>r.json()).then(d => d.content?.site && setSITE(s => ({...s, ...d.content.site}))).catch(()=>{}) }, [])
      ...
      <div className="bg-[#EF7B22] text-white text-xs">
        <div className="container flex items-center justify-between h-9">
          <div className="flex items-center gap-4">
            <span className="font-bold tracking-wide">Supermarket Konsum</span>
          </div>
