...
export async function ensureSeeded() {
  ...
  // Use DB flag to prevent concurrent seeds during dev hot-reload
  const flag = await db.collection('meta').findOne({ id: 'seed_flag' })
  if (flag?.done) { seeded = true; return }
  await db.collection('meta').updateOne({ id: 'seed_flag' }, { $set: { id: 'seed_flag', startedAt: new Date() } }, { upsert: true })
  ...
  seeded = true
  await db.collection('meta').updateOne({ id: 'seed_flag' }, { $set: { done: true, finishedAt: new Date() } }, { upsert: true })
}
